...
export default function App() {
  ...
  const [view, setView] = useState(storedToken && storedUser && storedUser !== 'null' ? 
    (JSON.parse(storedUser).role === 'startup' ? 'startup-dashboard' : 'investor-dashboard') : 
    'landing'
  );
  ...
  function handleAuth({ token, user }) {
    ...
    if (user.role === 'startup') setView('startup-dashboard');
          ...
          <div className="flex justify-center w-full">
            <div className="flex items-center justify-between w-full max-w-7xl px-6 py-4">
              <div 
                ...
                onClick={()=>go(token? (user?.role === 'startup' ? 'startup-dashboard' : 'investor-dashboard') : 'landing')}
              >
                <div className="flex items-center justify-center w-10 h-10 rounded-lg bg-[#0d93f2]/10 text-[#0d93f2]">
                  <span className="material-symbols-outlined text-2xl">rocket_launch</span>
                </div>
                <h2 className="text-slate-900 dark:text-white text-xl font-bold tracking-tight">FundConnect</h2>
              </div>
      ...
      {view !== 'investor-dashboard' && view !== 'startup-dashboard' && view !== 'landing' && view !== 'register' && view !== 'community' && view !== 'settings' && (
        <footer className="py-8 text-center border-t border-slate-200 dark:border-slate-800">
          <p className="text-xs text-slate-400">
            © {new Date().getFullYear()} FundConnect Inc. All rights reserved.
          </p>
